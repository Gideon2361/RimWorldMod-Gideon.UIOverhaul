...
namespace Gideon.UIOverhaul.Features.Ideoligions
{
    ...
    internal static class IdeoPanel
    {
        ...
        private static void Header(Rect rect, Ideo ideo, int faithsHere, UIColorPaletteDef palette)
        {
            ...
            TabParts.RowLabel(name, ideo.name, ideo.TextColor, GameFont.Medium);
        ...
        {
            float width = Mathf.Max(TabParts.PillWidth(value), TabParts.PillWidth(caption)) + 10f;
            ...
            UIColorPaletteDef palette)
        {
            ...
            y += 8f;
            y = TabParts.Heading(view, y, "Known elsewhere", palette);
            ...
            System.Func<Rect, float, float> body)
        {
            ...
            TabParts.RowLabel(new Rect(cap.x + 10f, cap.y, cap.width - 20f, capHeight), title, palette.TextPrimary);
        ...
        private static float Conviction(Rect view, float y, List<ConvictionRow> rows, UIColorPaletteDef palette)
        {
            ...
            return Block(view, y, "Conviction", suffix, palette, (inner, top) =>
            {
                ...
                for (int i = 0; i < rows.Count; i++)
                    cursor = ConvictionRowDraw(inner, cursor, rows[i], palette);

                return cursor;
            });
        }

        private static float ConvictionRowDraw(Rect inner, float y, ConvictionRow row, UIColorPaletteDef palette)
        {
            const float height = 22f;

            Rect band = new Rect(inner.x, y, inner.width, height);

            TabParts.RowLabel(new Rect(band.x, band.y, 110f, height), row.pawn.LabelShortCap, palette.TextPrimary);
            ...
            Rect bar = new Rect(band.x + 116f, band.y + 7f,
                Mathf.Max(40f, band.width - 116f - numberWidth - driftWidth - chipWidth - 12f), 8f);

            UIProgressBarControl.Draw(bar, row.certainty, palette, tint);

            TabParts.RowLabel(new Rect(bar.xMax + 4f, band.y, numberWidth, height),
                row.certainty.ToStringPercent("0"), palette.TextSecondary, GameFont.Tiny);
            ...
            TabParts.Pill(band, band.xMax - chipWidth, band.y + 2f, word, tint, palette);
        ...
        private static float Roles(Rect view, float y, List<RoleRow> rows, UIColorPaletteDef palette)
        {
            ...
            return Block(view, y, "Roles", suffix, palette, (inner, top) =>
            {
                ...
                for (int i = 0; i < rows.Count; i++)
                    cursor = RoleRowDraw(inner, cursor, rows[i], palette);

                return cursor;
            });
        }

        private static float RoleRowDraw(Rect inner, float y, RoleRow row, UIColorPaletteDef palette)
        {
            const float height = 22f;

            Rect band = new Rect(inner.x, y, inner.width, height);

            TabParts.RowLabel(new Rect(band.x, band.y, 96f, height), row.role.LabelCap, palette.Accent,
                GameFont.Tiny);
            ...
            float chipWidth = Mathf.Min(TabParts.PillWidth(chip) + 6f, band.width * 0.5f);

            TabParts.Pill(band, band.xMax - chipWidth, band.y + 2f, chip, tint, palette, chipWidth);

            TabParts.RowLabel(new Rect(band.x + 100f, band.y, band.width - 100f - chipWidth - 6f, height),
            ...
            UIColorPaletteDef palette)
        {
            ...
            return Block(view, y, "Obligations", suffix, palette, (inner, top) =>
            {
                ...
                for (int i = 0; i < rows.Count; i++)
                    cursor = ObligationRowDraw(inner, cursor, rows[i], palette);

                return cursor;
            });
        }

        private static float ObligationRowDraw(Rect inner, float y, ObligationRow row, UIColorPaletteDef palette)
        {
            const float height = 22f;

            Rect band = new Rect(inner.x, y, inner.width, height);

            TabParts.RowLabel(new Rect(band.x, band.y, 108f, height), row.ritual.LabelCap, palette.TextPrimary);
            ...
            TabParts.RowLabel(new Rect(band.x + 112f, band.y, band.width - 112f, height), text,
                row.owed ? palette.Warning : palette.TextSecondary, GameFont.Tiny);
        ...
        private static float DemandRowDraw(Rect inner, float y, DemandRow row, UIColorPaletteDef palette)
        {
            ...
            float chipWidth = Mathf.Min(TabParts.PillWidth(row.state) + 6f, 160f);

            TabParts.Pill(band, band.xMax - chipWidth, band.y + 2f, row.state, tint, palette, chipWidth);

            TabParts.RowLabel(new Rect(band.x, band.y, band.width - chipWidth - 8f, height),
                row.precept.LabelCap, palette.TextPrimary);
        ...
        private static float Doctrine(Rect view, float y, List<DoctrineRow> rows, UIColorPaletteDef palette)
        {
            ...
            return Block(view, y, "Doctrine", suffix, palette, (inner, top) =>
            {
                ...
                for (int i = 0; i < rows.Count; i++)
                    cursor = DoctrineRowDraw(inner, cursor, rows[i], palette);
        ...
        /// </summary>
        private static float DoctrineRowDraw(Rect inner, float y, DoctrineRow row, UIColorPaletteDef palette)
        {
            ...
            TabParts.RowLabel(new Rect(icon.xMax + 8f, band.y, 130f, height), row.issue, palette.TextSecondary,
                GameFont.Tiny);

            float impactWidth = 58f;

            TabParts.RowLabel(new Rect(band.x + 158f, band.y,
                band.width - 158f - impactWidth - 6f, height), row.stance, tint);
